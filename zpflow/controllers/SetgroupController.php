...
class SetgroupController extends BaseController{
	public function actionGetGroup(){
		...
		return $this->jsonReturn($groupInfo);
	}
	...
	public function actionSaveSetgroup(){
		...
		if(!empty($info)){
			if($info['gstID'] != "0"){
				...
				if($sgp->save()){
					return $this->jsonReturn(['result'=>1,'msg'=>'保存成功']);
				}else{
					return $this->jsonReturn(['result'=>0,'msg'=>'保存失败']);
				}
			}else{
	...
	public function actionDelSetgroup(){
		...
		if($flag > 0){
			$result = ['result'=>0,'msg'=>'该组别已经在使用中，不能删除'];
		}else{
