...
<div class="layui-side_sendmsg">
    <fieldset class="layui-elem-field layui-field-title">
	  	<legend>通知信息</legend>
	    <p id="stepIndex_flow4_step3_content" style="padding: 10px;"> </p>
	</fieldset>
</div>
  
<div class="layui-body_sendmsg">
    <div id="stepIndex_flow4_step3_sendmsg">
    	
    </div>
</div>
<?php $this->endBody() ?>
<script>
var __stepIndex_flow4_step3_sendmsg_recID = "<?php echo $recID; ?>";
$(function(){
	$("#stepIndex_flow4_step3_content").html(parent.__flow4_step3_msg_content__);
	$('#stepIndex_flow4_step3_sendmsg').datagrid({
		width:'auto',
        height:'520',
	    url:"<?= Url::to(['examiner/examiner-sendmsg-list']) ?>",
	    method: "get",
	    queryParams: {'recID':__stepIndex_flow4_step3_sendmsg_recID},
	    fixed: true,
	    fitColumns: false,
	    singleSelect:true,
	    rownumbers: true, 
...
function flow4_step3_send_msg_sure(){
	layui.use('layer', function(){
	 	var layer = layui.layer;
		var data = $("#stepIndex_flow4_step3_sendmsg").datagrid('getData');
		var rows = data.rows;
		var len = rows.length;
		var exmPhones = [];
		var exmContents = [];
		...
		$.ajax({
			...
			async:true,
			dataType:'json',
			data:{'exmPhones':exmPhones,'exmContents':exmContents,'content':parent.__flow4_step3_msg_content__},
