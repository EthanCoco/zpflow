	 ...
	 <tbody>
	    <tr>
	      	...
	      	<td>
	      		...
	      		<button type="button" class="layui-btn" id="flow6_file_btn"><i class="layui-icon"></i>选择文件</button>
	      	</td>
	    </tr>
	    <tr id="import_tr_sh" style="display: none;"><td colspan="2"><span id="showhtml"></span></td></tr>
	    <tr>
	      	<td>模板下载</td>
	      	<td><a href="javascript:void(0)" style="text-decoration:underline;color:blue;" onclick='window.open("<?= Url::to(['careful/importmb']) ?>"+"&recID=<?= $recID ?>");'>考生政审结果导入模板.xls</a></td>
	    </tr>
	</tbody>
</table>
...
<?php $this->endBody() ?>
<script>
var __flow6_import_recID__ = "<?= $recID ?>";	
$(function(){
	$("#fileName").val("");
	$("#showhtml").html("");
...
function flow6_import_data_sure(){
	layui.use(['upload','layer'], function(){
		if($("#fileName").val() == ""){
			return parent.layer.msg('请选择上传的文件');
		}
	    $.post("<?= Url::to(['careful/upexcel-sure']) ?>",{'recID':__flow6_import_recID__,'filePath':$("#fileName").val()},function(json){
			if(json.result){
				parent.init_stepIndex_six_grid();
